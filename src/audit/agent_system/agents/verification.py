...
class VerificationAgent(BaseAgent):
    ...
    def _parse_llm_response(self, response: str) -> VerificationStep:
        ...
        # 🔥 处理 XML 格式的响应
        if '<invoke>' in response or '<minimax:tool_call>>' in response:
            # 提取思考部分
            thought_match = re.search(r'(.*?)(?:<invoke>|<minimax:tool_call>>)', response, re.DOTALL)
            if thought_match:
                step.thought = thought_match.group(1).strip()
            else:
                step.thought = response[:500].strip()
            
            # 尝试提取 action
            action_match = re.search(r'<invoke name="(\w+)"', response)
            if not action_match:
                # 尝试从 <dispatch_agent> 标签获取
                dispatch_match = re.search(r'<dispatch_agent>', response)
                if dispatch_match:
                    step.action = "dispatch_agent"
                else:
                    # 尝试提取工具名称
                    tool_match = re.search(r'<(\w+)>', response)
                    if tool_match:
                        step.action = tool_match.group(1).strip()
            else:
            ...
            try:
                ...
                if tool_match:
                    ...
                    # 尝试提取 JSON
                    json_match = re.search(r'\{[\s\S]*\}', tool_content)
                    if json_match:
                        input_text = json_match.group(0).strip()
                        ...
                        # 尝试提取标签内容
                        step.action_input = {}
                        for tag in ['directory', 'file_path', 'query', 'pattern']:
                            tag_match = re.search(rf'<{tag}>(.*?)</{tag}>', tool_content, re.DOTALL)
                            if tag_match:
                                step.action_input[tag] = tag_match.group(1).strip()
            except Exception as e:
                logger.error(f"[Verification] 解析 XML 格式失败: {e}")
