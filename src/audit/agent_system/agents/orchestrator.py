...
请开始你的审计工作。首先思考应该如何开展，然后决定第一步做什么。"""
    ...
    def _parse_llm_response(self, response: str) -> Optional[AgentStep]:
        """解析 LLM 响应"""
        # 打印 LLM 原始输出，以便调试
        logger.debug(f"[Orchestrator] LLM 原始输出:\n{response}")
        
        # 检查是否是错误消息
        if "错误" in response or "timeout" in response.lower() or "超时" in response:
            logger.error(f"[Orchestrator] LLM 返回错误消息:\n{response}")
            # 尝试从错误消息中提取有用信息，并返回一个默认的 finish 操作
            return AgentStep(
                thought=f"LLM 服务返回错误: {response[:100]}...",
                action="finish",
                action_input={"conclusion": f"审计中断：{response}", "findings": [], "recommendations": []}
            )
        ...
        # 处理大写标签格式，如 <Thought> 和 </Action>
        cleaned_response = re.sub(r'<Thought>\s*(.*?)\s*</Thought>', r'Thought: \1', cleaned_response, flags=re.DOTALL)
        cleaned_response = re.sub(r'<Action>\s*(.*?)\s*</Action>', r'Action: \1', cleaned_response, flags=re.DOTALL)
        cleaned_response = re.sub(r'<Action Input>\s*(.*?)\s*</Action Input>', r'Action Input: \1', cleaned_response, flags=re.DOTALL)
        
        # 处理 minimax:tool_call 格式
        if '<minimax:tool_call>>' in cleaned_response or '<invoke>' in cleaned_response:
            # 提取思考部分（在 invoke 之前的内容）
            thought_match = re.search(r'(.*?)(?:<minimax:tool_call>>|<invoke>)', cleaned_response, re.DOTALL)
            thought = thought_match.group(1).strip() if thought_match else ""
            
            # 提取 invoke 内容
            invoke_match = re.search(r'(?:<minimax:tool_call>>|<invoke>)(.*?)(?:</invoke>|$)', cleaned_response, re.DOTALL)
            if invoke_match:
                invoke_content = invoke_match.group(1).strip()
                
                # 提取 action
                if 'dispatch_agent' in invoke_content:
                    action = "dispatch_agent"
                else:
                    action = "dispatch_agent"  # 默认 action
                
                # 提取 Action Input - 处理格式：agent: "recon", task: "...", context: "..."
                try:
                    # 提取 agent
                    agent_match = re.search(r'agent:\s*"([^"]*)"', invoke_content)
                    agent = agent_match.group(1).strip() if agent_match else "recon"
                    
                    # 提取 task
                    task_match = re.search(r'task:\s*"([^"]*)"', invoke_content, re.DOTALL)
                    task = task_match.group(1).strip() if task_match else ""
                    
                    # 提取 context
                    context_match = re.search(r'context:\s*"([^"]*)"', invoke_content, re.DOTALL)
                    ...
                    logger.debug(f"[Orchestrator] 从 minimax:tool_call 格式解析 Action Input: {action_input}")
                except Exception as e:
                    logger.error(f"[Orchestrator] 解析 minimax:tool_call 格式时出错: {e}")
                    # 尝试备选解析方法
                    action_input = {
                        "agent": "recon",
                        "task": invoke_content,
                    ...
                    logger.debug(f"[Orchestrator] 使用备选方法解析 Action Input: {action_input}")
            else:
                logger.error(f"[Orchestrator] 无法提取 invoke 内容")
                action = "dispatch_agent"
                action_input = {"agent": "recon", "task": "分析项目结构", "context": thought}
            
            logger.debug(f"[Orchestrator] 从 minimax:tool_call 格式提取 - Thought: {thought[:100]}..., Action: {action}")
            return AgentStep(
                thought=thought,
                action=action,
        ...
        # 打印清理后的响应
        logger.debug(f"[Orchestrator] 清理后的响应:\n{cleaned_response}")

        # 提取 Thought
        thought_match = re.search(r'Thought:\s*(.*?)(?=Action:|$)', cleaned_response, re.DOTALL)
        thought = thought_match.group(1).strip() if thought_match else ""
        logger.debug(f"[Orchestrator] 提取的 Thought: {thought[:100]}...")

        # 提取 Action
        action_match = re.search(r'Action:\s*(\w+)', cleaned_response)
        if not action_match:
            logger.error(f"[Orchestrator] 无法提取 Action，响应:\n{cleaned_response}")
            # 当无法提取 Action 时，返回一个默认的 finish 操作
            return AgentStep(
                thought="无法从 LLM 响应中提取操作指令",
                action="finish",
                action_input={"conclusion": "审计完成：无法获取 LLM 操作指令", "findings": [], "recommendations": []}
            )
        action = action_match.group(1).strip()
        logger.debug(f"[Orchestrator] 提取的 Action: {action}")

        # 提取 Action Input
        input_match = re.search(r'Action Input:\s*(.*?)(?=Thought:|Observation:|$)', cleaned_response, re.DOTALL)
        if not input_match:
            logger.error(f"[Orchestrator] 无法提取 Action Input，响应:\n{cleaned_response}")
            # 当无法提取 Action Input 时，返回一个默认的输入
            return AgentStep(
                thought=thought,
                action=action,
                action_input={"raw": "无法提取输入参数"}
            )
        ...
        logger.debug(f"[Orchestrator] 提取的 Action Input: {input_text[:100]}...")
        ...
        try:
            ...
            logger.debug(f"[Orchestrator] 解析后的 Action Input: {action_input}")
        except Exception as e:
            logger.error(f"[Orchestrator] 解析 Action Input 失败: {e}, 输入:\n{input_text}")
            # 当解析失败时，返回一个默认的输入
