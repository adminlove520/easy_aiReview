...
class FinishScanTool(AgentTool):
    ...
    def __init__(self, agent_state=None, tracer=None):
        super().__init__()
        self.agent_id = None
...
- 如果还有子Agent正在运行"""
    ...
    async def _execute(
        ...
        **kwargs
        ...
        # 动态获取根Agent ID
        self.agent_id = agent_registry.get_root_agent_id()
        if not self.agent_id:
            # 如果没有根Agent ID，尝试从注册表中获取第一个Agent ID
            tree = agent_registry.get_agent_tree()
            if tree.get('nodes'):
                self.agent_id = next(iter(tree['nodes'].keys()))
            else:
                return ToolResult(
                    success=False,
                    error="无法获取Agent ID，请确保Agent已正确注册"
                )
