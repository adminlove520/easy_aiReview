...
class Settings:
    def __init__(self):
        self.max_file_size_bytes = 5 * 1024 * 1024  # 5MB
        self.max_files_scan = 1000
        self.repo_scan_timeout_seconds = 3600
        self.GITHUB_TOKEN = os.getenv('GITHUB_TOKEN')
        self.GITLAB_TOKEN = os.getenv('GITLAB_TOKEN')
        self.GITEA_TOKEN = os.getenv('GITEA_TOKEN')
        self.ENABLE_SSH_KEYS = False
...
async def scan_repo_task(task_id: str, db_session_factory, user_config: dict = None):
    ...
    async with db_session_factory() as db:
        ...
        try:
            ...
            # 检查是否为SSH URL
            # 模拟GitSSHOperations类
            class MockGitSSHOperations:
                @staticmethod
                def is_ssh_url(url):
                    return url.startswith('git@') or url.startswith('ssh://')
                
                @staticmethod
                def get_repo_files_via_ssh(repo_url, ssh_private_key, branch, exclude_patterns):
                    return []
            
            GitSSHOperations = MockGitSSHOperations
