...
class Settings:
    def __init__(self):
        ...
        self.deepseek_api_model = os.getenv('DEEPSEEK_API_MODEL')
        self.zhipuai_api_key = os.getenv('ZHIPUAI_API_KEY')
        self.zhipuai_api_base_url = os.getenv('ZHIPUAI_API_BASE_URL')
        self.zhipuai_api_model = os.getenv('ZHIPUAI_API_MODEL')
        self.qwen_api_key = os.getenv('QWEN_API_KEY')
        self.qwen_api_base_url = os.getenv('QWEN_API_BASE_URL')
        self.qwen_api_model = os.getenv('QWEN_API_MODEL')
        self.minimax_api_key = os.getenv('MINIMAX_API_KEY')
        self.minimax_api_base_url = os.getenv('MINIMAX_API_BASE_URL')
        self.minimax_api_model = os.getenv('MINIMAX_API_MODEL')
        self.moonshot_api_key = os.getenv('MOONSHOT_API_KEY')
        self.moonshot_api_base_url = os.getenv('MOONSHOT_API_BASE_URL')
        self.moonshot_api_model = os.getenv('MOONSHOT_API_MODEL')
        self.baidu_api_key = os.getenv('BAIDU_API_KEY')
        self.baidu_api_base_url = os.getenv('BAIDU_API_BASE_URL')
        self.baidu_api_model = os.getenv('BAIDU_API_MODEL')
        self.doubao_api_key = os.getenv('DOUBAO_API_KEY')
        self.doubao_api_base_url = os.getenv('DOUBAO_API_BASE_URL')
        self.doubao_api_model = os.getenv('DOUBAO_API_MODEL')
...
class LLMService:
    ...
    def config(self) -> LLMConfig:
        ...
        if self._config is None:
            ...
            # 🔥 Model 优先级：用户配置 > 环境变量平台专属配置 > 环境变量通用配置 > 默认模型
            model = (
                user_llm_config.get('llmModel') or
                (getattr(settings, 'deepseek_api_model', '') if provider == LLMProvider.DEEPSEEK else '') or
                (getattr(settings, 'zhipuai_api_model', '') if provider == LLMProvider.ZHIPU else '') or
                (getattr(settings, 'qwen_api_model', '') if provider == LLMProvider.QWEN else '') or
                (getattr(settings, 'minimax_api_model', '') if provider == LLMProvider.MINIMAX else '') or
    ...
    def _get_provider_api_key(self, provider: LLMProvider) -> str:
        ...
        provider_key_map = {
            ...
            LLMProvider.ZHIPU: 'zhipuai_api_key',
    ...
    def _get_provider_base_url(self, provider: LLMProvider) -> Optional[str]:
        ...
        if provider == LLMProvider.OPENAI:
            ...
            return getattr(settings, 'minimax_api_base_url', None)
        elif provider == LLMProvider.DEEPSEEK:
            return getattr(settings, 'deepseek_api_base_url', None)
        elif provider == LLMProvider.ZHIPU:
            return getattr(settings, 'zhipuai_api_base_url', None)
        elif provider == LLMProvider.QWEN:
            return getattr(settings, 'qwen_api_base_url', None)
        elif provider == LLMProvider.MOONSHOT:
            return getattr(settings, 'moonshot_api_base_url', None)
        elif provider == LLMProvider.BAIDU:
            return getattr(settings, 'baidu_api_base_url', None)
        elif provider == LLMProvider.DOUBAO:
            return getattr(settings, 'doubao_api_base_url', None)
        return None
