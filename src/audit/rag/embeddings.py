...
class Settings:
    def __init__(self):
        ...
        # 根据LLM提供商自动选择相应的embedding provider
        self.embedding_provider = os.getenv('EMBEDDING_PROVIDER', self._get_default_embedding_provider())
        self.embedding_model = os.getenv('EMBEDDING_MODEL', self._get_default_embedding_model())
        
        # API密钥配置
        self.LLM_API_KEY = self._get_api_key()
    ...
    def _get_default_embedding_provider(self):
        """根据LLM提供商获取默认的embedding provider"""
        provider_map = {
            'openai': 'openai',
            'minimax': 'minimax',  # MiniMax使用自己的接口
            'zhipuai': 'zhipuai',  # 智谱AI使用自己的接口
            'deepseek': 'deepseek',  # DeepSeek使用自己的接口
            'qwen': 'qwen',  # 通义千问
            'ollama': 'ollama',  # Ollama本地
        }
        return provider_map.get(self.llm_provider.lower(), 'openai')
    
    def _get_default_embedding_model(self):
        """根据embedding provider获取默认的embedding model"""
        model_map = {
            'openai': 'text-embedding-3-small',
            'minimax': 'text-embedding-3-small',  # MiniMax使用OpenAI兼容接口
            'zhipuai': 'text-embedding-3-small',  # 智谱AI使用OpenAI兼容接口
            'deepseek': 'text-embedding-3-small',  # DeepSeek使用OpenAI兼容接口
        ...
        return model_map.get(self.embedding_provider.lower(), 'text-embedding-3-small')
    ...
    def _get_api_key(self):
        ...
        return api_key_map.get(self.llm_provider.lower(), os.getenv('OPENAI_API_KEY'))
...
class EmbeddingService:
    ...
    def _create_provider(
        ...
        if provider == "ollama":
            ...
            # MiniMax使用OpenAI兼容接口
            minimax_api_key = api_key or getattr(settings, 'minimax_api_key')
            minimax_base_url = base_url or getattr(settings, 'MINIMAX_API_BASE_URL', 'https://api.minimaxi.com/v1')
            return OpenAIEmbedding(api_key=minimax_api_key, base_url=minimax_base_url, model=model)

        elif provider == "zhipuai":
            # 智谱AI使用OpenAI兼容接口
            zhipuai_api_key = api_key or getattr(settings, 'zhipuai_api_key')
            zhipuai_base_url = base_url or getattr(settings, 'ZHIPUAI_API_BASE_URL', 'https://open.bigmodel.cn/api/paas/v4/')
            return OpenAIEmbedding(api_key=zhipuai_api_key, base_url=zhipuai_base_url, model=model)

        elif provider == "deepseek":
            # DeepSeek使用OpenAI兼容接口
            deepseek_api_key = api_key or getattr(settings, 'deepseek_api_key')
            deepseek_base_url = base_url or getattr(settings, 'DEEPSEEK_API_BASE_URL', 'https://api.deepseek.com')
            return OpenAIEmbedding(api_key=deepseek_api_key, base_url=deepseek_base_url, model=model)
