...
class ChromaVectorStore(VectorStore):
    ...
    async def update_collection_metadata(self, updates: Dict[str, Any]):
        ...
        try:
            ...
            # 创建一个不包含不可修改字段的元数据副本
            # "hnsw:space" 字段在 collection 创建后不能修改
            updatable_metadata = current_metadata.copy()
            if "hnsw:space" in updatable_metadata:
                del updatable_metadata["hnsw:space"]

            # Chroma 不支持直接更新元数据，需要通过修改 collection
            # 这里我们使用 modify 方法，只传递可更新的字段
            await asyncio.to_thread(
                self._collection.modify,
                metadata=updatable_metadata,
            )
        except Exception as e:
